...
public class BondLiveAndEODAPI {
		...
		throws Exception
	{
		...
		System.out.println ("Work-out Type for Price: " + org.drip.analytics.support.CurveProductHelper.WorkoutTypeToString (wi._iWOType));
		...
		System.out.println ("Next Exercise Type: " + org.drip.analytics.support.CurveProductHelper.WorkoutTypeToString (nei._iWOType));
		...
		for (LossPeriodCurveFactors dp : bond.getLossFlow (valParams, pricerParams, cmp))
			System.out.println (
				...
				GenericUtil.FormatSpreadSimple (dp.getCouponDCF(), 1, 4, 1.) + FIELD_SEPARATOR +
				GenericUtil.FormatSpreadSimple (dp.getEffectiveNotional(), 1, 0, 1.) + FIELD_SEPARATOR +
				GenericUtil.FormatSpreadSimple (dp.getEffectiveRecovery(), 1, 2, 1.) + FIELD_SEPARATOR +
				GenericUtil.FormatSpreadSimple (dp.getEffectiveDF(), 1, 4, 1.)  + FIELD_SEPARATOR +
				GenericUtil.FormatSpreadSimple (dp.getStartSurvival(), 1, 4, 1.) + FIELD_SEPARATOR +
				GenericUtil.FormatSpreadSimple (dp.getEndSurvival(), 1, 4, 1.)
			);
	}
