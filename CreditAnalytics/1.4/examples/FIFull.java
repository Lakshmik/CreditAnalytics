...
public class FIFull {
	...
	public static final org.drip.product.credit.Bond CreateCustomBond (final java.lang.String strName, final
		...
		if (org.drip.product.creator.BondBuilder.BOND_TYPE_SIMPLE_FLOATER == iBondType)
			bond = org.drip.product.creator.BondBuilder.CreateSimpleFloater (strName, "USD", "DRIPRI", 0.01,
				"30/360", org.drip.util.date.JulianDate.CreateFromYMD (2012, 3, 21),
			...
			bond = org.drip.product.creator.BondBuilder.CreateSimpleFixed (strName, "USD", 0.05, "30/360",
				org.drip.util.date.JulianDate.CreateFromYMD (2012, 3, 21),
					org.drip.util.date.JulianDate.CreateFromYMD (2023, 9, 20), MakeFSPrincipal(),
						MakeFSCoupon());
	...
	public static final void CDSAPISample() {
		...
		if (s_bCDSCouponCFDisplay) {
			...
			for (org.drip.analytics.period.ProductCouponPeriodCurveMeasures p : cds.getCouponFlow (valParams,
				pricerParams, org.drip.param.market.ComponentMarketParams.MakeCreditCMP
					(org.drip.analytics.curve.DiscountCurve.CreateFromFlatRate
						(org.drip.util.date.JulianDate.Today(), "USD", 0.05),
							org.drip.analytics.curve.CreditCurve.FromFlatHazard
								(org.drip.util.date.JulianDate.Today().getJulian(), "CC", 0.02, 0.4)))) {
				try {
	...
	public static final void BondAPISample() {
		...
		try {
			...
			dblDiscountMarginFromPrice = org.drip.service.api.FI.BondDiscountMarginFromPrice (strISIN,
				dtToday, dc, 1.);

			dblDiscountMarginTMFromPrice = org.drip.service.api.FI.BondDiscountMarginTMFromPrice (strISIN,
			...
			dblPECSFromPrice = org.drip.service.api.FI.BondPECSFromPrice (strISIN, dtToday, dc, cc, 1.);

			dblPECSTMFromPrice = org.drip.service.api.FI.BondCreditBasisTMFromPrice (strISIN, valParams, dc,
				cc, 1., quotingParams);
			...
			dblDiscountMarginFromTSYSpread = org.drip.service.api.FI.BondISpreadFromTSYSpread (strISIN,
		...
		try {
			bInFirstPeriod = org.drip.service.api.FI.InFirstPeriod (strISIN, valParams._dblValue);

			bInLastPeriod = org.drip.service.api.FI.InLastPeriod (strISIN, valParams._dblValue);
		} catch (java.lang.Exception e) {
		...
		if (s_bBondCouponCFDisplay) {
			...
			for (org.drip.analytics.period.ProductCouponPeriodCurveMeasures p : bond.getCouponFlow
				(valParams, pricerParams, org.drip.param.market.ComponentMarketParams.MakeCreditCMP
					(org.drip.analytics.curve.DiscountCurve.CreateFromFlatRate (dtToday, "USD", 0.05),
						org.drip.analytics.curve.CreditCurve.FromFlatHazard (dtToday.getJulian(), "CC", 0.02,
							0.4)))) {
				try {
					System.out.println (org.drip.util.date.JulianDate.fromJulian (p.getAccrualStartDate()) +
						"    " + org.drip.util.date.JulianDate.fromJulian (p.getAccrualEndDate()) + "    " +
			...
			try {
				System.out.println (strISIN + "    " + bond.getTicker() + " " +
					org.drip.util.common.FIGen.FormatPrice (bond.getCoupon
						(org.drip.util.date.JulianDate.Today().getJulian(),
							org.drip.param.market.ComponentMarketParams.MakeCreditCMP (dc, cc), null)) + " "
		...
		for (int i = 0; i < aBond.length; ++i) {
			...
			if (s_bCustomBondAnalDisplay) {
				...
				try {
					...
					System.out.println ("PECS From Price: " + org.drip.util.common.FIGen.FormatSpread
						(aBond[i].calcPECSFromPrice (org.drip.param.valuation.ValuationParams.CreateValParams
							(org.drip.util.date.JulianDate.Today(), 0, "USD",
								org.drip.analytics.daycount.DayCountBasis.DR_ACTUAL), cmp, null, wi._dblDate,
									wi._dblExerciseFactor, 1.)));
					...
					System.out.println ("PECS From TSY Spread: " + org.drip.util.common.FIGen.FormatSpread
						(aBond[i].calcPECSFromTSYSpread
							(org.drip.param.valuation.ValuationParams.CreateValParams
								(org.drip.util.date.JulianDate.Today(), 0, "USD",
									org.drip.analytics.daycount.DayCountBasis.DR_ACTUAL), cmp, null,
										0.0188)));
		...
		for (java.lang.String strISIN : lsstrISIN) {
			...
			if (null != bond && !bond.hasVariableCoupon() && !bond.hasBeenExercised() && !bond.hasDefaulted()
				...
				try {
					...
					if (!org.drip.service.api.FI.IsBondFloater (strISIN))
						dblZSpreadFromPrice = org.drip.service.api.FI.BondZSpreadFromPrice (strISIN,
							dtToday, dc, 1.);

					double dblDiscountMarginFromPrice = org.drip.service.api.FI.BondDiscountMarginFromPrice
					...
					try {
						dblCreditBasisFromPrice = org.drip.service.api.FI.BondCreditBasisFromPrice (strISIN,
							dtToday, dc, cc, 1.);
					} catch (java.lang.Exception e) {
					}

					try {
						dblPECSFromPrice = org.drip.service.api.FI.BondPECSFromPrice (strISIN, dtToday, dc,
							cc, 1.);
					} catch (java.lang.Exception e) {
					}
					...
					if (s_bTickerAnalDisplay)
						System.out.println (strISIN + "    " + bond.getTicker() + " " +
							org.drip.util.common.FIGen.FormatPrice (bond.getCoupon
								(org.drip.util.date.JulianDate.Today().getJulian(),
									org.drip.param.market.ComponentMarketParams.MakeCreditCMP (dc, cc),
										null)) + " " + bond.getMaturityDate() + "    " + (bond.isFloater() ?
											"FLOAT" : "FIXED") + "     " +
												org.drip.util.common.FIGen.FormatPrice (dblYieldFromPrice) +
							...
							(dblCreditBasisFromPrice) + "    " + (dblPECSFromPrice) + "    " +
								org.drip.util.common.FIGen.FormatPrice (dblBondCreditPrice));
				} catch (java.lang.Exception e) {
					if (s_bSupressErrMsg) {
	...
	public static final void BondEODMeasuresAPISample() {
		...
		org.drip.util.date.JulianDate dtEOD = org.drip.util.date.JulianDate.CreateFromYMD (2011, 12, 16);

		System.out.println ("Price measures for " + org.drip.service.api.FI.GetBondStringField (strISIN,
			"Description"));

		try {
			dblEODConvexityFromPrice = org.drip.service.api.FI.BondEODConvexityFromPrice (strISIN, dtEOD,
				dblEODPrice);

			try {
				dblEODCreditBasisFromPrice = org.drip.service.api.FI.BondEODCreditBasisFromPrice (strISIN,
					dtEOD, dblEODPrice);
			} catch (java.lang.Exception e) {
				e.printStackTrace();
			}
			...
			try {
				dblEODPECSFromPrice = org.drip.service.api.FI.BondEODPECSFromPrice (strISIN, dtEOD,
					dblEODPrice);
			} catch (java.lang.Exception e) {
				e.printStackTrace();
			}

			dblEODDiscountMarginFromPrice = org.drip.service.api.FI.BondEODDiscountMarginFromPrice (strISIN,
				dtEOD, dblEODPrice);

			dblEODDurationFromPrice = org.drip.service.api.FI.BondEODDurationFromPrice (strISIN, dtEOD,
				dblEODPrice);

			try {
				dblEODGSpreadFromPrice = org.drip.service.api.FI.BondEODGSpreadFromPrice (strISIN, dtEOD,
					dblEODPrice);
			} catch (java.lang.Exception e) {
				e.printStackTrace();
			}

			dblEODISpreadFromPrice = org.drip.service.api.FI.BondEODISpreadFromPrice (strISIN, dtEOD,
				dblEODPrice);

			dblEODOASFromPrice = org.drip.service.api.FI.BondEODOASFromPrice (strISIN, dtEOD, dblEODPrice);

			dblEODParASWFromPrice = org.drip.service.api.FI.BondEODParASWFromPrice (strISIN, dtEOD,
				dblEODPrice);

			try {
				dblEODTSYSpreadFromPrice = org.drip.service.api.FI.BondEODTSYSpreadFromPrice (strISIN, dtEOD,
					dblEODPrice);
			} catch (java.lang.Exception e) {
				e.printStackTrace();
			}

			dblEODYieldFromPrice = org.drip.service.api.FI.BondEODYieldFromPrice (strISIN,
				org.drip.util.date.JulianDate.CreateFromYMD (2011, 12, 16), dblEODPrice);

			dblEODZSpreadFromPrice = org.drip.service.api.FI.BondEODZSpreadFromPrice (strISIN, dtEOD,
				dblEODPrice);

			if (s_bBondEODMeasuresFromPrice) {
				System.out.println ("EOD Convexity From Price: " + dblEODConvexityFromPrice);

				System.out.println ("EOD Credit Basis From Price: " + org.drip.util.common.FIGen.FormatSpread
					(dblEODCreditBasisFromPrice));

				System.out.println ("EOD PECS From Price: " + org.drip.util.common.FIGen.FormatSpread
					(dblEODPECSFromPrice));

				System.out.println ("EOD Discount Margin From Price: " + org.drip.util.common.FIGen.FormatSpread
					(dblEODDiscountMarginFromPrice));

				System.out.println ("EOD Duration From Price: " + dblEODDurationFromPrice);

				System.out.println ("EOD G Spread From Price: " + org.drip.util.common.FIGen.FormatSpread
					(dblEODGSpreadFromPrice));

				System.out.println ("EOD I Spread From Price: " + org.drip.util.common.FIGen.FormatSpread
					(dblEODISpreadFromPrice));

				System.out.println ("EOD OAS From Price: " + org.drip.util.common.FIGen.FormatSpread
					(dblEODOASFromPrice));

				System.out.println ("EOD Par ASW From Price: " + (int) dblEODParASWFromPrice);

				System.out.println ("EOD TSY Spread From Price: " + org.drip.util.common.FIGen.FormatSpread
					(dblEODTSYSpreadFromPrice));

				System.out.println ("EOD Yield From Price: " + org.drip.util.common.FIGen.FormatPrice
					(dblEODYieldFromPrice));

				System.out.println ("EOD Z Spread From Price: " + org.drip.util.common.FIGen.FormatSpread
					(dblEODZSpreadFromPrice));
			}
		} catch (java.lang.Exception e) {
			if (s_bSupressErrMsg) {
		...
		System.out.println ("\nYield measures for " + org.drip.service.api.FI.GetBondStringField (strISIN,
			"Description"));

		try {
			dblEODConvexityFromYield = org.drip.service.api.FI.BondEODConvexityFromYield (strISIN, dtEOD,
				dblEODYield);

			try {
				dblEODCreditBasisFromYield = org.drip.service.api.FI.BondEODCreditBasisFromYield (strISIN,
					dtEOD, dblEODYield);
			} catch (java.lang.Exception e) {
				if (s_bSupressErrMsg) {
					System.out.println ("BondEODMeasuresAPISample failed.");
			...
			try {
				dblEODPECSFromYield = org.drip.service.api.FI.BondEODPECSFromYield (strISIN, dtEOD,
					dblEODYield);
			} catch (java.lang.Exception e) {
				if (s_bSupressErrMsg) {
					System.out.println ("BondEODMeasuresAPISample failed.");
			...
			dblEODDiscountMarginFromYield = org.drip.service.api.FI.BondEODDiscountMarginFromYield (strISIN,
				dtEOD, dblEODYield);

			dblEODDurationFromYield = org.drip.service.api.FI.BondEODDurationFromYield (strISIN, dtEOD,
				dblEODYield);

			try {
				dblEODGSpreadFromYield = org.drip.service.api.FI.BondEODGSpreadFromYield (strISIN, dtEOD,
					dblEODYield);
			} catch (java.lang.Exception e) {
				if (s_bSupressErrMsg) {
					System.out.println ("BondEODMeasuresAPISample failed.");
			...
			dblEODISpreadFromYield = org.drip.service.api.FI.BondEODISpreadFromYield (strISIN, dtEOD,
				dblEODYield);

			dblEODOASFromYield = org.drip.service.api.FI.BondEODOASFromYield (strISIN, dtEOD, dblEODYield);

			dblEODParASWFromYield = org.drip.service.api.FI.BondEODParASWFromYield (strISIN, dtEOD,
				dblEODYield);

			dblEODPriceFromYield = org.drip.service.api.FI.BondEODPriceFromYield (strISIN, dtEOD,
				dblEODYield);

			try {
				dblEODTSYSpreadFromYield = org.drip.service.api.FI.BondEODTSYSpreadFromYield (strISIN, dtEOD,
					dblEODYield);
			} catch (java.lang.Exception e) {
				if (s_bSupressErrMsg) {
					System.out.println ("BondEODMeasuresAPISample failed.");
			...
			dblEODZSpreadFromYield = org.drip.service.api.FI.BondEODZSpreadFromYield (strISIN, dtEOD,
				dblEODYield);

			if (s_bBondEODMeasuresFromYield) {
				System.out.println ("EOD Convexity From Yield: " + dblEODConvexityFromYield);

				System.out.println ("EOD Credit Basis From Yield: " + org.drip.util.common.FIGen.FormatSpread
					(dblEODCreditBasisFromYield));

				System.out.println ("EOD PECS From Yield: " + org.drip.util.common.FIGen.FormatSpread
					(dblEODPECSFromYield));

				System.out.println ("EOD Discount Margin From Yield: " +
						org.drip.util.common.FIGen.FormatSpread (dblEODDiscountMarginFromYield));

				System.out.println ("EOD Duration From Yield: " + dblEODDurationFromYield);

				System.out.println ("EOD G Spread From Yield: " + org.drip.util.common.FIGen.FormatSpread
					(dblEODGSpreadFromYield));

				System.out.println ("EOD I Spread From Yield: " + org.drip.util.common.FIGen.FormatSpread
					(dblEODISpreadFromYield));

				System.out.println ("EOD OAS From Yield: " + org.drip.util.common.FIGen.FormatSpread
					(dblEODOASFromYield));

				System.out.println ("EOD Par ASW From Yield: " + (int) dblEODParASWFromYield);

				System.out.println ("EOD Price From Yield: " + org.drip.util.common.FIGen.FormatPrice
					(dblEODPriceFromYield));

				System.out.println ("EOD TSY Spread From Yield: " + org.drip.util.common.FIGen.FormatSpread
					(dblEODTSYSpreadFromYield));

				System.out.println ("EOD Z Spread From Yield: " + org.drip.util.common.FIGen.FormatSpread
					(dblEODZSpreadFromYield));
			}
		} catch (java.lang.Exception e) {
			if (s_bSupressErrMsg) {
				System.out.println ("BondEODMeasuresAPISample failed.");
		...
		System.out.println ("\nTSY Spread measures for " + org.drip.service.api.FI.GetBondStringField
			(strISIN, "Description"));

		try {
			dblEODConvexityFromTSYSpread = org.drip.service.api.FI.BondEODConvexityFromTSYSpread (strISIN,
				dtEOD, dblEODTSYSpread);

			try {
				dblEODCreditBasisFromTSYSpread = org.drip.service.api.FI.BondEODCreditBasisFromTSYSpread
			...
			try {
				dblEODPECSFromTSYSpread = org.drip.service.api.FI.BondEODPECSFromTSYSpread (strISIN, dtEOD,
					dblEODTSYSpread);
			} catch (java.lang.Exception e) {
				if (s_bSupressErrMsg) {
					System.out.println ("BondEODMeasuresAPISample failed.");
			...
			dblEODDiscountMarginFromTSYSpread = org.drip.service.api.FI.BondEODDiscountMarginFromTSYSpread
				(strISIN, dtEOD, dblEODTSYSpread);

			dblEODDurationFromTSYSpread = org.drip.service.api.FI.BondEODDurationFromTSYSpread (strISIN,
				dtEOD, dblEODTSYSpread);

			try {
				dblEODGSpreadFromTSYSpread = org.drip.service.api.FI.BondEODGSpreadFromTSYSpread (strISIN,
					dtEOD, dblEODTSYSpread);
			} catch (java.lang.Exception e) {
				if (s_bSupressErrMsg) {
					System.out.println ("BondEODMeasuresAPISample failed.");
			...
			dblEODISpreadFromTSYSpread = org.drip.service.api.FI.BondEODISpreadFromTSYSpread (strISIN, dtEOD,
				dblEODTSYSpread);

			dblEODOASFromTSYSpread = org.drip.service.api.FI.BondEODOASFromTSYSpread (strISIN, dtEOD,
				dblEODTSYSpread);

			dblEODParASWFromTSYSpread = org.drip.service.api.FI.BondEODParASWFromTSYSpread (strISIN, dtEOD,
				dblEODTSYSpread);

			dblEODPriceFromTSYSpread = org.drip.service.api.FI.BondEODPriceFromTSYSpread (strISIN, dtEOD,
				dblEODTSYSpread);

			try {
				dblEODYieldFromTSYSpread = org.drip.service.api.FI.BondEODYieldFromTSYSpread (strISIN, dtEOD,
					dblEODTSYSpread);
			} catch (java.lang.Exception e) {
				if (s_bSupressErrMsg) {
					System.out.println ("BondEODMeasuresAPISample failed.");
			...
			dblEODZSpreadFromTSYSpread = org.drip.service.api.FI.BondEODZSpreadFromTSYSpread (strISIN, dtEOD,
				dblEODTSYSpread);

			if (s_bBondEODMeasuresFromTSYSpread) {
				System.out.println ("EOD Convexity From TSY Spread: " + dblEODConvexityFromTSYSpread);

				System.out.println ("EOD Credit Basis From TSY Spread: " + org.drip.util.common.FIGen.FormatSpread
					(dblEODCreditBasisFromTSYSpread));

				System.out.println ("EOD PECS From TSY Spread: " + org.drip.util.common.FIGen.FormatSpread
					(dblEODPECSFromTSYSpread));

				System.out.println ("EOD Discount Margin From TSY Spread: " +
					org.drip.util.common.FIGen.FormatSpread (dblEODDiscountMarginFromTSYSpread));
				...
				System.out.println ("EOD G Spread From TSY Spread: " +
					org.drip.util.common.FIGen.FormatSpread (dblEODGSpreadFromTSYSpread));

				System.out.println ("EOD I Spread From TSYSpread: " + org.drip.util.common.FIGen.FormatSpread
					(dblEODISpreadFromTSYSpread));

				System.out.println ("EOD OAS From TSY Spread: " + org.drip.util.common.FIGen.FormatSpread
					(dblEODOASFromTSYSpread));

				System.out.println ("EOD Par ASW From TSY Spread: " + (int) dblEODParASWFromTSYSpread);

				System.out.println ("EOD Price From TSY Spread: " + org.drip.util.common.FIGen.FormatPrice
					(dblEODPriceFromTSYSpread));

				System.out.println ("EOD Yield From TSY Spread: " + org.drip.util.common.FIGen.FormatSpread
					(dblEODYieldFromTSYSpread));

				System.out.println ("EOD Z Spread From TSY Spread: " +
					org.drip.util.common.FIGen.FormatSpread (dblEODZSpreadFromTSYSpread));
			}
		} catch (java.lang.Exception e) {
			if (s_bSupressErrMsg) {
				System.out.println ("BondEODMeasuresAPISample failed.");
	...
	public static final void CDSEODMeasuresAPISample() {
		org.drip.util.date.JulianDate dtEOD = org.drip.util.date.JulianDate.CreateFromYMD (2011, 7, 21);

		org.drip.product.credit.CreditDefaultSwap cds = org.drip.product.credit.CreditDefaultSwap.CreateSNAC
				(org.drip.util.date.JulianDate.Today(), "5Y", 0.1, "813796");

		java.util.Map<java.lang.String, java.lang.Double> mapEODCDSMeasures =
			org.drip.service.api.FI.GetEODCDSMeasures (cds, dtEOD);
		...
		if (s_bCDSBondCreditCurve) {
			try {
				System.out.println (cds.getPrimaryCode() + " => " + cds.calcMeasureValue (valParams,
					org.drip.param.pricer.PricerParams.MakeStdPricerParams(), new
						org.drip.param.market.ComponentMarketParams (dc, null, null, ccCalib, null, null,
							org.drip.util.common.FIGen.CreateFixingsObject (bond,
								org.drip.util.date.JulianDate.Today(), 0.04)), null, "FairPremium"));

				System.out.println (bond.getPrimaryCode() + " => " + bond.calcPriceFromCreditBasis
					(valParams, new org.drip.param.market.ComponentMarketParams (dc, null, null, ccCalib,
						null, null, org.drip.util.common.FIGen.CreateFixingsObject (bond,
							org.drip.util.date.JulianDate.Today(), 0.04)), null,
								bond.getMaturityDate().getJulian(), 1., 0.));
			} catch (java.lang.Exception e) {
				e.printStackTrace();
			}
	...
	public static final void BasketBondAPISample() {
		...
		if (s_bBasketBond) {
			System.out.println ("Fair Clean Price: " + org.drip.util.common.FIGen.FormatPrice (mapResult.get
				("FairCleanPV")));

			System.out.println ("Fair Yield: " + org.drip.util.common.FIGen.FormatPrice (mapResult.get
				("Yield")));

			System.out.println ("Fair GSpread: " + org.drip.util.common.FIGen.FormatSpread (mapResult.get
				("FairGSpread")));

			System.out.println ("Fair ZSpread: " + org.drip.util.common.FIGen.FormatSpread (mapResult.get
				("FairZSpread")));

			System.out.println ("Fair ISpread: " + org.drip.util.common.FIGen.FormatSpread (mapResult.get
				("FairISpread")));
	...
	public static final void BasketCDSAPISample() {
		...
		org.drip.product.credit.BasketProduct bpCDX = org.drip.service.api.FI.MakeCDX ("CDX.NA.IG", 17,
			"5Y");

		org.drip.product.credit.BasketProduct bpCDXOTR = org.drip.service.api.FI.MakeCDX ("CDX.NA.IG",
			dtToday, "5Y");

		java.util.Set<java.lang.String> setstrCDXNames =
			org.drip.product.credit.StandardCDXManager.GetCDXNames();

		java.util.Map<java.lang.String, java.lang.String> mapCDXDescr =
			org.drip.product.credit.StandardCDXManager.GetCDXDescriptions();

		org.drip.product.credit.BasketProduct bpPresetOTR =
			org.drip.product.credit.StandardCDXManager.GetOnTheRun ("CDX.EM", dtToday.subtractTenor ("1Y"),
				"5Y");

		org.drip.product.credit.BasketProduct bpPreLoadedOTR =
			org.drip.product.credit.StandardCDXManager.GetOnTheRun ("ITRAXX.ENERGY", dtToday.subtractTenor
				("7Y"), "5Y");

		java.util.Map<org.drip.util.date.JulianDate, java.lang.Integer> mapCDXSeries =
			org.drip.product.credit.StandardCDXManager.GetCDXSeriesMap ("ITRAXX.ENERGY");
		...
		if (s_bCDXSeries) {
			for (java.util.Map.Entry<org.drip.util.date.JulianDate, java.lang.Integer> me :
