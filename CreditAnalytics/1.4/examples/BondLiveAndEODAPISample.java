...
public class BondLiveAndEODAPISample {
	...
	public static final void BondTickerAPISample() throws Exception {
		...
		for (String strISIN : lsstrISIN) {
			...
			if (null != bond && !bond.hasVariableCoupon() && !bond.hasBeenExercised() && !bond.hasDefaulted()
				...
				double dblPECSFromPrice = FI.BondCreditBasisFromPrice (strISIN, dtToday, dc, cc, 1.);
				...
				System.out.println (strISIN + FIELD_SEPARATOR +
					...
					FIGen.FormatSpread (dblPECSFromPrice) + FIELD_SEPARATOR +
	...
	public static final void BondAPISample() throws Exception {
		...
		double dblPECSFromPrice = FI.BondPECSFromPrice (strISIN, dtToday, dc, cc, 0.5);

		double dblPECSTMFromPrice = FI.BondPECSTMFromPrice (strISIN, valParams, dc, cc, 0.5, quotingParams);
		...
		double dblPECSFromTSYSpread = FI.BondCreditBasisFromTSYSpread (strISIN, dtToday, dc, dcTSY, cc, 0.0971);
