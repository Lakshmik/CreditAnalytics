...
public class FIFull {
	...
	private static final boolean s_bBondEODMeasures = false;
	...
	public static final void BondEODMeasuresAPISample() {
		...
		double dbEODYield = java.lang.Double.NaN;

		try {
			dbEODYield = org.drip.service.api.FI.BondEODYieldFromPrice (strISIN,
				org.drip.util.date.JulianDate.CreateFromYMD (2011, 12, 16), 1.);

			if (s_bBondEODMeasures) {
	...
	public static void BondCDSCurveCalibration() {
		...
		if (s_bCDSBondCreditCurve) {
			for (int i = 0; i < aCompCDS.length; ++i)
				System.out.println (aCompCDS[i].getPrimaryCode() + " => " + adblCDSQuotes[i]);
		}
	}
	...
	public static final void BasketBondAPISample() {
		...
		if (s_bBasketBond) {
			...
			System.out.println ("Fair Yield: " + mapResult.get ("Yield"));
